...
const props = defineProps(["id", "selected", "current_id", "text", "text_duration"]);
...
watch(
	() => [props.text, props.text_duration],
	() => {
		if (props.id === props.current_id) {
			localStates.value.text = props.text;
			localStates.value.text_duration = props.text_duration;
		}
	}
);
			...
			<div data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" @click="handleData" class="main-container"
				...
				<div class="content">
					<div class="card" style="width: 18rem; text-align: center">
						<div class="card-body">
							<p class="card-title"><i class="las la-comment-alt"></i> {{ getMixinValue.lang.text_message }}</p>
							<span class="d-none"> Delay : {{ localStates.text_duration }}</span>
